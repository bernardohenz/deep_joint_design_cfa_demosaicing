...
parser.add_argument('--dir',required=True, help='specify the dir with the images to be reconstructed')
...
print('Starting predictions on dir: ',str(opt.dir))

if (opt.output_dir is not None):
   if not os.path.exists(opt.output_dir):
      os.makedirs(opt.output_dir)
    
imgs = glob.glob(opt.dir+'/*')
...
for i,img_name in enumerate(imgs):
   ...
   if (opt.output_dir is not None):
      scipy.misc.toimage(predicted,cmin=0,cmax=255).save(img_name.replace(opt.dir,opt.output_dir))
   psnrs[i] = cpsnr(img,predicted)
print("{:s} - psnr: {:.2f} time : {:.2f} seg".format(opt.dir,np.mean(psnrs),np.mean(reject_outliers(times))))
